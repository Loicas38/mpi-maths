...
\textbf{2. Par l'image réciproque :}
...
\textbf{3. Par caractérisation séquentielle :}
...
\textbf{4. Autre méthode (voisinage) :}
...
\begin{correction}{5}
 \textbf{a) Distance nulle et adhérence :}
...
On sait que $\forall a,b \in \mathbb{R}, (|a|-|b|)^{2} \ge 0 \implies a^{2}+b^{2} \ge 2|ab|$.
Donc $|ab| \le \frac{1}{2}(a^{2}+b^{2})$.
Et $\sqrt{|xy|} \le \frac{1}{\sqrt{2}} \sqrt{x^{2}+y^{2}}$.
Cependant, l'inégalité utile ici est plutôt : $\sqrt{|x|} \le \sqrt{|x|+|y|}$ et $\sqrt{|y|} \le \sqrt{|x|+|y|}$.
...
2. $\varphi_{2}(P) = \int_{0}^{1} P(t) dt = \sum_{i=0}^{n} \frac{a_{i}}{i+1}$.
...
1. Soit $g : x \mapsto \|x-f(x)\|$. $g$ est continue sur le compact $A$, donc elle atteint son minimum en un point $x_{0}$.
Supposons $g(x_{0}) > 0$.
...
2. $\|u_{n+1} - x_{0}\| = \|f(u_{n}) - f(x_{0})\| \le k \|u_{n} - x_{0}\|$.
